	...
	for sourceID, severity := range vuln.VendorSeverity {
		...
		if cvss, ok := vuln.CVSS[sourceID]; ok {
			...
			if cvss.V40Score != 0 || cvss.V40Vector != "" {
				rates = append(rates, m.ratingV40(sourceID, severity, cvss))
			}
...
func (m *Marshaler) ratingV40(sourceID dtypes.SourceID, severity dtypes.Severity, cvss dtypes.CVSS) cdx.VulnerabilityRating {
	return cdx.VulnerabilityRating{
		Source: &cdx.Source{
			Name: string(sourceID),
		},
